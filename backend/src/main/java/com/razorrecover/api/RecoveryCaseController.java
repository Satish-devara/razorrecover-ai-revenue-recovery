...
public class RecoveryCaseController {
    ...
    public ResponseEntity<RecoveryCaseResponse> open(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(recoveryCaseService.open(request, OperationContext.from(idempotencyKey)));
    }
    ...
    public List<RecoveryDecisionResponse> decisions(@PathVariable UUID recoveryCaseId) {
        return recoveryCaseService.decisions(recoveryCaseId);
    }

    @GetMapping("/{recoveryCaseId}/audit-events")
    public List<AuditEventResponse> auditEvents(@PathVariable UUID recoveryCaseId) {
